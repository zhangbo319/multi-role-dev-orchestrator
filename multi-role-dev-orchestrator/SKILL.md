...
description: 适用于需要将单个需求拆分为多个有依赖关系的研发角色，并由编排器按依赖图分层调度的场景。默认模型为 product、architecture、development、testing、synthesis，其中 development 与 testing 可并行，不用于开放式头脑风暴或自由多 agent 讨论。
---
...
使用这个 skill 时，优先把问题建模为一个依赖图驱动的多 Agent 流程，而不是自由聊天式多 agent 讨论。默认模型是 `product -> architecture -> (development, testing) -> synthesis`，目标是让每个角色都产生明确文档产物，并由汇总角色收敛最终结论。
...
其中 `development` 和 `testing` 同层并行，`synthesis` 最后汇总所有上游产物。
...
- `orchestrate.py`：创建运行目录并按依赖图分层调度多个角色
